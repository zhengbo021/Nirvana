...
function defaultExclude(workingDirectory: string) {
  return new vscode.RelativePattern(
    workingDirectory,
    "**/{node_modules,dist,build,out,test,.vscode, .git}/**",
  );
}

export async function getValidTsFileNames(
  ...
  const fileNames = files.map((file) =>
    file.fsPath.replace(workingDirectory + "/", ""),
  );
  return fileNames;
}

export async function getEnvFileNames(
  workingDirectory: string,
): Promise<string[]> {
